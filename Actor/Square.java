...
class Square {
	...
	*/
	...
	// solve is NOT used in this PDA example
	while ( true ) {
	    ...
	    // more here
	    /* Numerous additional tactics have been added in later versions.
	       {These solvers are used for the >generation< of puzzles
	       that are >human< solvable.  Depth first search solves all
	       puzzles but these may require backtracking, which is not 
	       'allowed' for regular puzzles. */
...
class Tile {
	...
	for ( int k = 1; k < 10; k++ ) 
	    if ( !elementsX[k] ) {
		System.out.println("last " + ixi + " " + ixj + " -> " + k);
		setVal(k);
		break;
	    }
    } // end last

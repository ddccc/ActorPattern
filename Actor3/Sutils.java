...
public class Sutils {
    ...
    // Checks for Tile elements
    static public boolean check(Square s) {
	 boolean out;
	 for (int i = 0; i < 9; i++ ) {
	     out = checkRow(s.getRow(i));
	     if ( !out ) return false;
	 }
	 for (int j = 0; j < 9; j++ ) {
	     out = checkColumn(s.getCol(j));
	     if ( !out ) return false;
	 }
	 for (int i = 0; i < 3; i++) {
	     for (int j = 0; j < 3; j++) { 
		 out = checkArr(s.getArr(i, j));
		 if ( !out ) return false;
	     }
	 }
	 return true;
     }
     static public boolean checkRow(Seq row) {
	 boolean [] arb = new boolean[10]; 
	 for (int j = 0; j < 9; j++ ) {
	     int val = row.getTile(j).getVal();
	     if ( 0 == val ) continue;
	     if ( arb[val] ) return false;
	     arb[val] = true;
	 }
     ...
     static public boolean checkColumn(Seq col) {
	 boolean [] arb = new boolean[10]; 
	 for (int i = 0; i < 9; i++ ) {
	     int val = col.getTile(i).getVal();
	     if ( 0 == val ) continue;
	     if ( arb[val] ) return false;
	     arb[val] = true;
	 }
     ...
     static public boolean checkArr(Arr arr) {
	 boolean [] arb = new boolean[10]; 
	 for ( int p = 0; p < 3; p++ ) {
	     for ( int q = 0; q < 3; q++ ) {
		 int val = arr.getTile(p, q).getVal();
		 if ( 0 == val ) continue;
		 if ( arb[val] ) return false;
		 arb[val] = true;
	     }

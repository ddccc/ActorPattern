// File c:/ddc/Java/Actor/Square.java
...
public class Square {
    ...
    public void makeInStack() {
	Vector tiles = new Vector();
	for ( int i = 0; i < 9; i++ )
	    for ( int j = 0; j < 9; j++ ) {
		Tile t = getTile(i, j);
		int val = t.getVal();
		if ( 0 == val ) { // skip assigned tiles
		    t.calcSumZeroCnt(); // sum of zero's in row, column and arr
		    tiles.addElement(t);
		}
	    }
	Collections.sort(tiles); // high-low ordering
	int lng = tiles.size();
	/*
	  The ordering is high-low (23-16).  Thus we push the high ones first on
	  the stack.  The low one's are the most constrained thus it is best to 
	  start with them first when applying depth first search  
	*/
	for ( int i = 0; i < lng; i++ ) inStack.push(tiles.elementAt(i));
	///* // show what is on this stack:
	  for ( int z = 0; z < lng; z++ ) {
	    Tile t = (Tile) tiles.elementAt(z);
	    System.out.println(
		"i " + t.getI() +
		" j " + t.getJ() +
		" val " + t.getVal() + // must be zero
		" sum " + t.getSumZeroCnt() );
	  }
	  System.out.println("lng " + lng + " size " + inStack.size() );
...
class Seq extends RegionValues { 
	...
	if ( square.cols[j1].inRegion(b) ||
	    ...
	    System.out.print("completeRow2 A ");
	    tiles[j1].setVal(a);
	    tiles[j2].setVal(b);
	    return;
	}
	if ( square.cols[j1].inRegion(a) || 
	    ...
	    System.out.print("completeRow2 B ");
	    tiles[j1].setVal(b);
	    tiles[j2].setVal(a);
	    return;
	}
	...
	if ( ap1q1V1.mustCol(j1, b) || ap1q1V2.mustCol(j1, b) || 
	    ...
	    System.out.print("completeRow2 A2 ");
	    tiles[j1].setVal(a);
	    tiles[j2].setVal(b);
	    return;
	} 
	...
	if ( ap1q1V1.mustCol(j1, a) || ap1q1V2.mustCol(j1, a) || 
	    ...
	    System.out.print("completeRow2 A3 ");
	    tiles[j1].setVal(b);
	    tiles[j2].setVal(a);
	    return;
	} 
	...
	if ( ap1q2.inRegion(a) || 
	     ap1q1.inRegion(b) ) {
	    System.out.print("completeRow2 C ");
	    tiles[j1].setVal(a);
	    tiles[j2].setVal(b);
	    return;
	}
	if ( ap1q1.inRegion(a) ||
	     ap1q2.inRegion(b) ) {
	    System.out.print("completeRow2 D ");
	    tiles[j1].setVal(b);
	    tiles[j2].setVal(a);
	    return;
	}
	...
	if ( square.rows[i2].inRegion(a) ||
	    ...
	    System.out.print("completeCol2 A ");
	    tiles[i1].setVal(a);
	    tiles[i2].setVal(b);
	    return;
	}
	if ( square.rows[i1].inRegion(a) ||
	    ...
	    System.out.print("completeCol2 B ");
	    tiles[i1].setVal(b);
	    tiles[i2].setVal(a);
	    return;
	}
	...
	if ( ap1q1H1.mustRow(i1, b) || ap1q1H2.mustRow(i1, b) || 
	    ...
	    System.out.print("completeCol2 A2 ");
	    tiles[i1].setVal(a);
	    tiles[i2].setVal(b);
	    return;
	} 
	...
	if ( ap1q1H1.mustRow(i1, a) || ap1q1H2.mustRow(i1, a) || 
	    ...
	    System.out.print("completeCol2 A3 ");
	    tiles[i1].setVal(b);
	    tiles[i2].setVal(a);
	    return;
	} 
	...
	if ( ap2q1.inRegion(a) ||
	     ap1q1.inRegion(b) ) {
	    System.out.print("completeCol2 C ");
	    tiles[i1].setVal(a);
	    tiles[i2].setVal(b);
	    return;
	}
	if ( ap1q1.inRegion(a) ||
	     ap2q1.inRegion(b) ) {
	    System.out.print("completeCol2 D ");
	    tiles[i1].setVal(b);
	    tiles[i2].setVal(a);
	    return;
	}
...
class Arr extends RegionValues { 
	...
	if ( square.rows[i2].inRegion(a) ||
	     square.cols[j2].inRegion(a) ) {
	    System.out.print("completeArr2 A ");
	...
	if ( square.rows[i1].inRegion(b) ||
	    ...
	    System.out.print("completeArr2 B ");
	    tiles[p1][q1].setVal(a);
	    tiles[p2][q2].setVal(b);
	    return;
	}

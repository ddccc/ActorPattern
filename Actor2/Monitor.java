// File: c:/ddc/Java/Actor2/Monitor.java
...
class MonitorGoDown extends Monitor {
    public MonitorGoDown (Actor actor, State nextAction) { 
	super(actor, nextAction); 
    }
    public void check() {
	if ( !state.getStringValue().equals("goDown") ) return;
	check0();
	...
	state.setIntValue(1); 
	state.setStringValue("testVal");
	state.wakeUp(); // wakes up monitorTestVal
    } // end check()
} // end MonitorGoDown

class MonitorTestVal extends Monitor {
    public MonitorTestVal (Actor actor, State nextAction) { 
	super(actor, nextAction); 
    }
    public void check() {
	if ( ! state.getStringValue().equals("testVal") ) return;
	check0();
	Actor actor = (Actor) actorMonitored;
	Stack outStack = actor.getOutstack();	
	/* // testing
	...
	*/
	...
	if ( notOK ) {
	    ...
	    state.setStringValue("goDown");
	}
	state.wakeUp(); // wakes up monitor monitorGoDown or monitorIncrementQ
    } // end check()
} // end MonitorTestVal

class MonitorIncrementQ extends Monitor {
    public MonitorIncrementQ (Actor actor, State nextAction) { 
	super(actor, nextAction); 
    }
    public void check() {
	if ( ! state.getStringValue().equals("incrementQ") ) return;
	check0();
	Actor actor = (Actor) actorMonitored;
	int val = state.getIntValue();
	// Tile t = (Tile) outStack.peek();
	...
	state.wakeUp(); // wakes up monitorGoUp or monitorTestVal
    } // end check()
} // end MonitorIncrementQ

class MonitorGoUp extends Monitor {
    public MonitorGoUp (Actor actor, State nextAction) { 
	super(actor, nextAction);
    }
    public void check() {
	if ( ! state.getStringValue().equals("goUp") ) return;
	check0();
	Actor actor = (Actor) actorMonitored;
	Stack outStack = actor.getOutstack();
	if ( outStack.empty() ) {
	...
	state.setIntValue(val); // next value up will be tried
	state.setStringValue("incrementQ");
